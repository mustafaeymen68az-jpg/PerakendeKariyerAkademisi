...
import { Users, GraduationCap, Crown, UserCheck, ArrowRight, Sparkles, BarChart3 } from 'lucide-react';

export default function HomeRoleTabs() {
  const [activeTab, setActiveTab] = useState<'calisan' | 'egitim_muduru' | 'ik' | 'ceo' | 'egitmen'>('calisan');

  return (
    <div id="rol-sekmeleri" className="space-y-8">
      {/* Central 5 Role Selector Buttons Bar in Upper-Middle */}
      <div className="flex flex-wrap justify-center items-center gap-3 bg-[#0B2A4A]/5 p-3 rounded-3xl border border-[#0B2A4A]/10 max-w-5xl mx-auto shadow-sm">
        
        {/* 1. ÖĞRENCİ / ÇALIŞAN */}
        <button
          onClick={() => setActiveTab('calisan')}
          className={`flex items-center space-x-2 px-4 py-3 rounded-2xl font-black text-xs transition-all shadow-md cursor-pointer ${
            activeTab === 'calisan'
        ...
        {/* 2. EĞİTİM MÜDÜRÜ */}
        <button
          onClick={() => setActiveTab('egitim_muduru')}
          className={`flex items-center space-x-2 px-4 py-3 rounded-2xl font-black text-xs transition-all shadow-md cursor-pointer ${
            activeTab === 'egitim_muduru'
              ? 'bg-indigo-600 text-white shadow-xl scale-105 ring-2 ring-white'
              : 'bg-white/80 text-[#0B2A4A] hover:bg-white'
          }`}
        >
        ...
        >
          ...
          <span>👑 CEO / Genel Müdür Rolü</span>
        </button>
      ...
      <div className="transition-all duration-300">
        ...
        {activeTab === 'egitim_muduru' && (
          <div className="space-y-6">
            <div className="bg-indigo-50 border border-indigo-200 rounded-3xl p-6 max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
              <div>
                <span className="text-[10px] font-mono font-black text-indigo-800 uppercase tracking-wider block">EĞİTİM MÜDÜRÜ BAKIŞ AÇISI</span>
                <h3 className="text-xl font-black text-indigo-950">Eğitim Yönetimi & LMS Kontrol Merkezi</h3>
                <p className="text-xs text-gray-600 mt-1">
                  SCORM/xAPI uyumlu ders paketleri, akademi soru bankası, otomatik eğitim atamaları ve eğitim sonrası saha KPI etki analizi.
                </p>
              </div>
              <Link
                href="/ik-cozumlari/egitim-yonetimi"
                className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs rounded-2xl shadow-lg whitespace-nowrap flex items-center space-x-1.5"
              >
                <span>Tam Ekran Eğitim Müdürü Portalına Git</span>
                <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
        ...
        {activeTab === 'ceo' && (
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-700 text-white rounded-3xl p-6 max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
              <div>
                <span className="text-[10px] font-mono font-black text-amber-400 uppercase tracking-wider block">CEO & GENEL MÜDÜR BAKIŞ AÇISI</span>
          ...
          <div className="space-y-6">
            <div className="bg-purple-50 border border-purple-200 rounded-3xl p-6 max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
              <div>
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Ders modülü yönetimi, sınav oluşturma, ödev takibi ve Eğitmen SWOT analiz yönetim araçları.
                </p>
              </div>
              <Link
                href="/egitmen"
