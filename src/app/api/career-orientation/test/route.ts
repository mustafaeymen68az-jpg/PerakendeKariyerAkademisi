...
// Questions & Options seed data according to user prompt verbatim
...
async function ensureSeededTest() {
  let test = await prisma.careerOrientationTest.findFirst({
    where: { active: true },
    include: {
      questions: {
  ...
  if (!test || !test.questions || test.questions.length === 0) {
    let testId = test?.id;
    if (!testId) {
      const created = await prisma.careerOrientationTest.create({
        data: {
      ...
      await prisma.careerOrientationQuestion.deleteMany({
        where: { testId }
      });
    }
    ...
    // Refetch loaded test with seeded relations
    test = await prisma.careerOrientationTest.findUnique({
      where: { id: testId },
      include: {
        questions: {
          include: { options: true },
...
export async function GET(req: Request) {
  try {
    ...
    const test = await ensureSeededTest();

    // Check if user has an ongoing or previous attempt
    let activeAttempt = null;
    let previousResult = null;

    if (userId) {
      activeAttempt = await prisma.userCareerOrientationAttempt.findFirst({
        where: {
          userId,
          testId: test!.id,
          status: { in: ['IN_PROGRESS', 'TIE_BREAKER'] }
        },
        include: {
          answers: true
        }
      ...
      const completedAttempt = await prisma.userCareerOrientationAttempt.findFirst({
        where: {
          userId,
          testId: test!.id,
          status: 'COMPLETED'
      ...
      if (completedAttempt && completedAttempt.result) {
        previousResult = {
    ...
    return NextResponse.json({
      ...
      test: {
        ...
        cooldownMonths: test!.cooldownMonths,
      ...
      activeAttempt: activeAttempt ? {
        ...
        answers: activeAttempt.answers.map(a => ({
    ...
    console.error('Career Orientation GET error:', error);
    return NextResponse.json({ success: false, error: 'Test verisi yüklenemedi.' }, { status: 500 });
  }
}
